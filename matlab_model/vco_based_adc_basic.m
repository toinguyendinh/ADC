...
%f_vco = (a*V_in^3 + ab*V_in^2 + c*V_in + fc)*10^9; %approximate function of VCRO (Hz)
...
for i = 1:1:length(t_s)
    a = sampling_time*(i-1) +1;
    b = sampling_time*i;
    c = a:b;
    square_wave = square(2*pi*f_vco(i)*t(c));
    VCO = [VCO, square_wave]; %Voltage signal of VCO
